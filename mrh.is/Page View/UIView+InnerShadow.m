...
- (void)drawInnerShadowInRect:(CGRect)rect radius:(CGFloat)radius fillColor:(UIColor *)fillColor;
{
    ...
    CGPathAddArc(visiblePath, NULL, bounds.size.width-radius, bounds.size.height-radius, radius, 0.5f*M_PI, 0.0f*M_PI, YES);
    CGPathAddArc(visiblePath, NULL, bounds.size.width-radius, radius, radius, 0.0f*M_PI, 1.5f*M_PI, YES);
    CGPathAddArc(visiblePath, NULL, radius, radius, radius, 1.5f*M_PI, 1.0f*M_PI, YES);
    CGPathAddArc(visiblePath, NULL, radius, bounds.size.height-radius, radius, 1.0f*M_PI, 0.5f*M_PI, YES);
    ...
    CGContextSetShadowWithColor(context, CGSizeMake(0.0f, -1.0f), 12.0f, [shadowColor CGColor]);
    [shadowColor setFill];   
